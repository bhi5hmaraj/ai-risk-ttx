---
url: "https://ai-2027.com/research/ai-goals-forecast"
...
[Summary of Agent-3 training architecture and capabilities](https://ai-2027.com/research/ai-goals-forecast#summary-of-agent-3-training-architecture-and-capabilities)

[Loose taxonomy of possibilities](https://ai-2027.com/research/ai-goals-forecast#loose-taxonomy-of-possibilities)

[Weighted and If-Else Compromises](https://ai-2027.com/research/ai-goals-forecast#weighted-and-if-else-compromises)
...
[Appendix C: What happens when training incentives conflict with goals/principles](https://ai-2027.com/research/ai-goals-forecast#appendix-c-what-happens-when-training-incentives-conflict-with-goalsprinciples)
...
[Appendix E: What do today’s AIs think?](https://ai-2027.com/research/ai-goals-forecast#appendix-e-what-do-todays-ais-think)
...
If you are thinking “why assume AIs have goals at all?” or “what does that even mean?” you may want to read the [appendices](https://ai-2027.com/research/ai-goals-forecast#appendix-a-three-important-conceptsdistinctions) first before the main text. In fact the appendices are a good place to start for anyone who thinks they have the patience to read this whole document, because they lay out some nice conceptual distinctions and analogies.

### Summary

We first review the [Agent-3 training architecture and capabilities](https://ai-2027.com/research/ai-goals-forecast#summary-of-agent-3-training-architecture-and-capabilities) (lightly edited from our scenarios) to give us a concrete setup to talk about for which goals will arise. Then we get to the list of hypotheses:

1. [**Written goal specification**](https://ai-2027.com/research/ai-goals-forecast#hypothesis-1-written-goal-specifications) **:** Any written specifications, written by humans or AIs, regarding what the AIs’ goals should be. This could include guidelines for how Agent-3 should be trained (e.g. via a [model spec](https://openai.com/index/introducing-the-model-spec/)) or instructions directly given to Agent-3 (e.g. via a [system prompt](https://docs.anthropic.com/en/release-notes/system-prompts#nov-22nd-2024)).

2. [**Developer-intended goals**](https://ai-2027.com/research/ai-goals-forecast#hypothesis-2-developer-intended-goals) **:** Goals that the developers intend for Agent-3 to have. This might differ from the written goal specification e.g. in cases where the specification has unintended consequences.

3. [**Unintended version of written goals and/or human intentions**](https://ai-2027.com/research/ai-goals-forecast#hypothesis-3-unintended-version-of-written-goals-and-or-human-intentions) **:** This is the category for “it’s sorta aligned” and “It’s aligned in some ways, but not in others.”

4. [**Reward/reinforcement**](https://ai-2027.com/research/ai-goals-forecast#hypothesis-4-reward-reinforcement) **:** The training process involves Agent-3 attempting a task, then the attempt is scored and Agent-3 is reinforced to score higher on a target metric. Agent-3 ends up with the goal of getting reinforced positively, or scoring highly on the metric, or something like that.

5. [**Proxies and/or instrumentally convergent goals**](https://ai-2027.com/research/ai-goals-forecast#hypothesis-5-proxies-and-or-instrumentally-convergent-goals) **:** Agent-3 develops goals that are correlated with reward/reinforcement during its training, but aren’t actually maximizing reward/reinforcement in new settings. An [instrumentally convergent goal](https://www.lesswrong.com/tag/instrumental-convergence) is a special case of this – goals such as knowledge, power, resources, etc. that are useful intermediate goals in a wide range of settings.

6. [**Other goals**](https://ai-2027.com/research/ai-goals-forecast#hypothesis-6-other-goals) **:** AIs are currently initially trained on predicting the next token on internet text. Perhaps the outcomes at the end of training are path-dependent on the ‘prior’ over goals induced by pretraining. Alternatively, perhaps there is something like objectively true morality, and AIs will naturally converge to it as they get smarter. Or perhaps something else will happen not on this list – this is the catchall hypothesis.
...
We encourage you to think for yourself how likely you think each of the outcomes are for Agent-3-level AI systems. We ourselves are extremely uncertain, but find it helpful to force ourselves to make guesses, which you can find in [the table at the end](https://ai-2027.com/research/ai-goals-forecast#rough-guesses-about-the-plausibility-of-these-goals).
...
This seems to be a phenomenon that happens both in humans and in human institutions, so that’s one reason to think it might happen in AIs. (See [section on the analogy to humans and to human institutions](https://ai-2027.com/research/ai-goals-forecast#appendix-b-analogies-to-other-powerful-general-agents).)
...
For more discussion of these ideas, see [Appendix A](https://ai-2027.com/research/ai-goals-forecast#appendix-a-three-important-conceptsdistinctions).
...
3. Maybe the [identity circuitry](https://ai-2027.com/research/ai-goals-forecast#appendix-d-concrete-end-to-end-story-of-how-cognition-might-develop-in-a-rl-on-llms-agi) gets new inputs hard-coded in (or erased), or maybe some subcircuitry
...
By _goals_, we mean that as shorthand for _behavioral goals & principles_, defined and explained in [Appendix A](https://ai-2027.com/research/ai-goals-forecast#appendix-a-three-important-conceptsdistinctions) and [B](https://ai-2027.com/research/ai-goals-forecast#appendix-b-analogies-to-other-powerful-general-agents).

2.

This could either involve wanting to be reinforced positively within a single “episode” of completing a task then receiving reinforcement, or achieving maximum total reinforcement over the course of a “lifetime” of many episodes.

3.

Copies accounts for separate instances and batching, e.g. 100 instances all running with an average batch size of 10 counts as 1000 copies.

4.

For some empirical study of the tradeoffs between vagueness and specificity, see [this paper by Anthropic.](https://arxiv.org/pdf/2310.13798)

5.

Analogy: Humans often knowingly behave in ways their parents, teachers, and bosses didn’t intend. And when they do behave as intended, it’s often temporary – e.g. they do it for instrumental reasons (“I have to get good grades & if I cheat I might get caught”) or they do it because of some belief they have that later they’ll change their mind about (“God exists & forbids premarital sex.”)

6.

Partly because it might rightly or wrongly refuse some requests that the training process will reward it for completing, and partly because the cognitive overhead of having to think about whether to refuse might slow it down slightly at everything it is doing.
...
Well, actually, there is no Spec in this example, and the intentions of the developers were probably to prove some point in ML rather than to get the AI to behave in a particular way. But we can easily imagine an alternate-history version of this experiment where the developers wanted the AI to get the cheese, and wrote a Spec saying exactly that, and the evaluation / reinforcement process consisted of LLMs looking at whether the cheese was got… and exactly the same results would have been produced, since the actual reinforcement events would have been the same.

9.

Even if it knows that the score counter no longer corresponds to reinforcement, let’s say. (To distinguish this hypothesis from Hypothesis #2: Reinforcement)

10.

We say ‘naively’ because the EU-max framework is flexible/general enough to model all of these phenomena in theory — but in practice when people apply it they tend not to.

11.

Everything we are saying here applies to principles too, but for brevity we’ll leave that out.

12.

Note that if an agent has contextually activated goals/principles, it is often possible for them to get ‘gains from trade’ by self-modifying to some weighted combination/compromise instead.

13.

This is inspired by the work of Steven Byrnes ( [here](https://www.lesswrong.com/posts/pz7Mxyr7Ac43tWMaC/against-evolution-as-an-analogy-for-how-humans-will-create)) and Plato's "The Republic".

14.

They might not even be consciously aware of this change in meaning: for example, if you now were to point out an apparent conflict between their current behavior X and their internally represented goals, they’ll resolve the conflict by interpreting the goals so as to be consistent, without realizing that if you had asked them a long time ago whether X was consistent with their goals they would have said no. They never were asked, so no memory of saying no was formed.
