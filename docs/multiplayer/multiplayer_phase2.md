...
### Colyseus Express: Small, Focused Custom Route

Add the snapshot HTTP surface on the Colyseus server itself. This is idiomatic and keeps the authoritative owner (the room) close to the route.

Using @colyseus/tools (initializeExpress):
...
export default config({
  initializeGameServer: (gameServer) => {
    gameServer.define('game', GameRoom).filterBy(['gameId']);
  },

  initializeExpress: (app) => {
    app.get('/games/:gameId/snapshot', async (req, res) => {
      const { gameId } = req.params;
      try {
        const rooms = await matchMaker.query({ name: 'game', gameId });
        if (!rooms.length) return res.status(404).json({ error: 'game_not_found' });
        const roomId = rooms[0].roomId;
        const snapshot = await matchMaker.remoteRoomCall(roomId, 'getSnapshot');
...
Manual Express (current repo pattern in `server/index.ts`) is equivalent:

```ts
import { matchMaker } from 'colyseus';
expressApp.get('/games/:gameId/snapshot', async (req, res) => {
  const { gameId } = req.params;
  try {
    const rooms = await matchMaker.query({ name: 'game', gameId });
    if (!rooms.length) return res.status(404).json({ error: 'game_not_found' });
...
Your room exposes a read‑only method:
...
export class GameRoom extends Room<GameState> {
  ...
  getSnapshot() {
    ...
    const players = this.stateManager.getCorePlayers();
    return buildSnapshot(core, players); // phase, round, maxRounds, coreMetric, public players, deadlineAt, small tails
  }
}
```

Why on Colyseus, not Next? The snapshot is “authoritative live room state”. `remoteRoomCall()` is designed for this; putting the route on the Colyseus HTTP server avoids inventing a back‑channel from Next.

Minimal routes to ship now:
- `GET /games/:gameId/snapshot` (SSR/admin/replay bootstrap)
- (Optional, protected) `POST /games/:gameId/admin/advance` for testing overrides

Division of labor with Mail/Events:
- Colyseus Express: live snapshot (may include a tiny recentMail tail if desired)
- Next + Postgres: read‑heavy endpoints for mail threads and event history

### WS vs Snapshot Reconciliation
- Source of truth: the Room’s StateManager. Both WS patches and the snapshot read from it (use `remoteRoomCall('getSnapshot')` cross‑process).
- Precedence: while connected, prefer WS. Snapshots are for SSR bootstrap and hard reloads only.
- Versioning (recommended for SSR): include `stateVersion` (monotonic) and/or `eventSeq` (monotonic) in the snapshot and WS state.
  - Hydration: compare `ws.stateVersion` vs `ssr.stateVersion`; adopt the newer.
- No in‑session snapshot fetches: any runtime gaps are handled by Colyseus reconnect/resync; the browser does not call the snapshot endpoint to repair deltas.
- Timers: include `deadlineAt` (epoch ms) in snapshot; server enforces timeouts. Client timers are visual only and re‑sync on patch.
- Caching: respond with `Cache-Control: no-store` and optionally `ETag` tied to `stateVersion` to avoid accidental stale caches.

### Snapshot vs WebSocket — Stock App Analogy

| Channel  | Analogy (Stocks)             | Primary uses                                  | Payload                                             | Pros                                                 | Caveats |
|---------|-------------------------------|-----------------------------------------------|-----------------------------------------------------|------------------------------------------------------|--------|
| Snapshot| Opening positions/order book  | Next SSR first paint; hard reload; read‑only  | phase, round, players(public), scores, lastK events, stateVersion, eventSeq, deadlineAt | Deterministic first render; redaction; admin views   | Heavier; not live; no caching (no‑store) |
| WebSocket| Live price ticks/book deltas | Low‑latency gameplay updates; continuous state | Schema patches + discrete events (with eventSeq)    | Tiny payloads; real‑time; built‑in reconnect/resync  | Ticks can be missed; rely on reconnect or snapshot |

Summary: snapshot gets you “on the book,” WebSocket keeps you live. If you miss ticks, rely on Colyseus reconnect/resync; use snapshot only for SSR or hard reloads.
...
Client Glue Updates (part of server‑driven loop)
- Provider (providers/ColyseusProvider.tsx)
  - Remove `onMessage('all_submitted', ...)` auto‑advance.
  - Remove `advanceRound` from the exposed API; keep `startGame`, `submitAction`, `setRole`.
  - Keep state sync and message handlers (`round_result`, `new_round`, `action_options`, `players_init`, `current_event`, `game_ended`).
- Hook (hooks/useGameActionsColyseus.ts)
  - Stop calling `advanceRound()` after submit; rely on server broadcasts.
  - Keep AP pre‑check and optimistic flags; loading is gated by server messages (`action_options`).
...
5. Client navigates to `/game/[roomCode]` with generated room code (e.g., ABC123).
6. RoleSelector component renders (user not connected yet).
7. User selects role + enters name, clicks "Join Game".
8. Client connects to Colyseus with `gameId`, creates new room.
9. WaitingRoom component renders (phase === 'lobby').
...
Join Game (Joiner Flow)
1. User receives room code/link from friend.
2. Opens link → lands on `/game/ABC123`.
3. SSR renders basic page structure (not connected yet).
4. RoleSelector component renders (user not connected).
5. User selects an available role + enters name.
6. Clicks "Join Game".
7. Client calls `colyseusConnect({ gameId: 'ABC123', role: 'Journalist', name, isHuman: true })`.
8. Colyseus matchmaker routes to existing room with the same `gameId`.
9. WaitingRoom component renders (phase === 'lobby'), showing all connected players.
...
**Role Selection Flow** (Updated):
1. User lands on `/game/ABC123` (from shared link or direct navigation)
2. If not connected, show RoleSelector component (full-screen, not modal)
3. User selects role + enters name → clicks "Join Game"
4. Client calls `colyseusConnect({ gameId, role, name, isHuman: true })`
5. Server creates/joins room, adds player to Schema
6. Once connected, WaitingRoom component renders (phase === 'lobby')

**Implementation Notes**:
- RoleSelector lives at `/game/[roomCode]` route level (not in lobby)
- Uses existing RoleSelector component from `components/game/RoleSelector.tsx`
- Available roles fetched from server state or hardcoded for MVP
- Lobby page (`/game/page.tsx`) may be deprecated in favor of direct room links
