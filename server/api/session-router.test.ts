...
import { createValidGameSetup } from '@/tests/fixtures/session-data';
...
describe('session-router (pure)', () => {
  it('POST /session creates a new session', async () => {
    ...
    const { res, body } = await parse(await handleSessionRequest('POST', [], {}, { mode: 'classic', setup: createValidGameSetup() }, deps));
  ...
  it('POST /session rejects invalid setup with 400', async () => {
    const deps = makeTestRouterDeps();
    const badSetup = { scenarioTitle: 't', coreMetric: { name: 'Trust', description: 'desc' } }; // missing value
    const { res, body } = await parse(await handleSessionRequest('POST', [], {}, { mode: 'classic', setup: badSetup as any }, deps));
  ...
  it('GET /session/:id returns 304 with If-None-Match', async () => {
    const deps = makeTestRouterDeps();
    const created = await parse(await handleSessionRequest('POST', [], {}, { mode: 'classic', setup: createValidGameSetup() }, deps));
