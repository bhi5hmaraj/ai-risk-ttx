...
[Method 1: Time horizon extension](https://ai-2027.com/research/timelines-forecast#method-1-time-horizon-extension)

[Method 2: Benchmarks and gaps](https://ai-2027.com/research/timelines-forecast#method-2-benchmarks-and-gaps)
...
![](https://ai-2027.com/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fcombined-headline.4c72f6dd.png&w=3840&q=75)ai-2027.com _Forecast for the arrival of superhuman coders assuming no large-scale catastrophes happen (e.g., a solar flare, a pandemic, nuclear war), no government or self-imposed slowdown, and no significant supply chain disruptions. All forecasts give a substantial chance of superhuman coding arriving by and in 2027._

## Summary

_2025 May 7 update: Eli has, based on feedback, made a few updates to the model that he’s described in [the appendix](https://ai-2027.com/research/timelines-forecast#2025-may-7-update). The updates noticeably but not greatly lengthen his timelines,_ _now putting lower but still substantial probability on superhuman coders by 2027. The appendix also contains [an analysis of the importance of superexponentiality in the model](https://ai-2027.com/research/timelines-forecast#the-importance-of-each-source-of-superexponential-progress)._

We forecast when the leading AGI company will internally develop a [_superhuman coder (SC)_](https://ai-2027.com/research/timelines-forecast#defining-a-superhuman-coder-sc): an AI system that can do any coding tasks that the best AGI company engineer does, while being much faster and cheaper. At this point, the SC will likely speed up AI progress substantially as is explored in our [takeoff forecast](https://ai-2027.com/research/takeoff-forecast).

We first show **Method 1**: [time-horizon-extension](https://ai-2027.com/research/timelines-forecast#method-1-time-horizon-extension), a relatively simple model which forecasts when SC will arrive by extending the trend established by [METR’s report](https://arxiv.org/pdf/2503.14499) of AIs accomplishing tasks that take humans increasing amounts of time.

We then present **Method 2**: [benchmarks-and-gaps](https://ai-2027.com/research/timelines-forecast#method-2-benchmarks-and-gaps), a more complex model starting from a forecast saturation of an AI R&D benchmark ( [RE-Bench](https://metr.org/blog/2024-11-22-evaluating-r-d-capabilities-of-llms/)), and then how long it will take to go from that system to one that can handle real-world tasks at the best AGI company.
...
| --- | --- | --- | --- |
| [Time-horizon-extension model](https://ai-2027.com/research/timelines-forecast?tab=t.0#heading=h.ym96g6tkhuod) (Apr 2025) | 2027 (2025 to 2039) | 2027 (2025 to 2033) | N/A |
| [Updated time horizon model](https://ai-2027.com/research/timelines-forecast#2025-may-7-update) (May 2025) | 2029 (2026 to 2052) | N/A | N/A |
| [Benchmarks-and-gaps model](https://ai-2027.com/research/timelines-forecast?tab=t.0#heading=h.nc76pumi87i1) (Apr 2025) | 2028 (2025 to >2050) | 2027 (2025 to 2044) | 2032 (2026 to >2050) |
| [Updated benchmarks and gaps model](https://ai-2027.com/research/timelines-forecast#2025-may-7-update) (May 2025) | 2030 (2026 to 2095) | N/A | N/A |
...
In [the appendix](https://ai-2027.com/research/timelines-forecast#the-importance-of-each-source-of-superexponential-progress) we present the effects of a few important model parameters. The code for our simulation is [here](https://github.com/uvafan/timelines-takeoff-ai-2027).
...
2. [When will this time horizon and reliability be reached?](https://ai-2027.com/research/timelines-forecast#forecasting-scs-arrival). This is broken down into:
...
4. Accounting for [intermediate speedups](https://ai-2027.com/research/timelines-forecast#intermediate-speedups) and the internal-public gap
...
We first discuss what the main [gaps](https://ai-2027.com/research/timelines-forecast#what-are-the-gaps-in-task-difficulty-between-re-bench-saturation-and-sc) are in “task difficulty” between RE-Bench saturation and SC. Then we describe our [methodology](https://ai-2027.com/research/timelines-forecast#methodology) for forecasting how long it will take to cross these gaps.Then we [forecast how quickly all gaps in task difficulty will be crossed](https://ai-2027.com/research/timelines-forecast#how-fast-can-the-task-difficulty-gaps-be-crossed).
...
1. Define a milestone that would indicate the gap being crossed, which is strictly harder from the previous milestone such that the time between the gaps must be positive. These look like “Same as above, but… \[the gap has been crossed, e.g. it can do all the tasks above 30x faster and cheaper\]” and can be viewed in [this summary table](https://ai-2027.com/research/timelines-forecast#summary-table).
...
3. Find the time to cross all task difficulty gaps by incorporating [intermediate speedups](https://ai-2027.com/research/timelines-forecast#intermediate-speedups) of AI progress, then add other [potential slowdowns](https://ai-2027.com/research/timelines-forecast#compute-scaling-and-algorithmic-progress-slowdown) (e.g. adoption lag) and account for the [gap between internal and external deployment](https://ai-2027.com/research/timelines-forecast#gap-between-internal-and-external-deployment).
...
Below we summarize our gap crossing forecasts. FutureSearch below refers to the aggregate of 2 professional forecasters from [FutureSearch](https://futuresearch.ai/). Detailed individual rationales for each gap are in the [appendix](https://ai-2027.com/research/timelines-forecast#individual-forecaster-views-for-benchmark-gap-model-factors).

| Gap name | Milestone that would indicate the gap being crossed | Predictions for gap size (median and 80% CI) | Reasoning summary |
| --- | --- | --- | --- |
| [Time horizon: Achieving tasks that take humans lots of time.](https://ai-2027.com/research/timelines-forecast#time-horizon) | Ability to develop a wide variety of software projects involved in the AI R&D process which involve modifying a maximum of 10,000 lines of code across files totaling up to 20,000 lines. Clear instructions, unit tests, and other forms of ground-truth feedback are provided. Do this for tasks that take humans about 1 month (as controlled by the “initial time horizon” parameter) with 80% reliability, add the same cost and speed as humans. | Eli: 18 \[2, 144\] Nikola: 16 \[1, 125\] (these aren’t lognormal as they’re simulated; see more [below](https://ai-2027.com/research/timelines-forecast#time-horizon)) FutureSearch: 12.7 \[1.7, 48\] | Calculated from needed horizon length and doubling time. |
| [Engineering complexity: Handling complex codebases](https://ai-2027.com/research/timelines-forecast#engineering-complexity-handling-complex-codebases) | Ability to develop a wide variety of software projects involved in the AI R&D process which involve modifying >20,000 lines of code across files totaling up to >500,000 lines. Clear instructions, unit tests, and other forms of ground-truth feedback are provided. Do this for tasks that take humans about 1 month (as controlled by the “initial time horizon” parameter) with 80% reliability, add the same cost and speed as humans. | Eli: 3 \[0.5, 18\] Nikola: 3 \[0.5, 18\] FutureSearch: 11 \[2.4, 33.9\] | Estimated via performance trends on METR’s time horizon task suite. |
| [Feedback loops: working without externally provided feedback](https://ai-2027.com/research/timelines-forecast#feedback-loops-working-without-externally-provided-feedback) | Same as above, but without provided unit tests and only a vague high-level description of what the project should deliver. | Eli: 6 \[0.8, 45\] Nikola: 3 \[0.5, 18\] FutureSearch: 18.3 \[1.7, 58\] | Estimated from looking at how much removing Best-of-K sampling from RE-Bench diminishes performance. |
| [Parallel projects: handling several interacting projects](https://ai-2027.com/research/timelines-forecast#parallel-projects-handling-several-interacting-projects) | Same as above, except working on separate projects spanning multiple codebases that interface together (e.g., a large-scale training pipeline, an experiment pipeline, and a data analysis pipeline). | Eli: 1.4 \[0.5, 4\] Nikola: 1.2 \[0.5, 3\] FutureSearch: 2 \[0.7, 5.3\] | Estimated as being very small due to overlap with the engineering complexity and time horizon gaps. |
| [Specialization: Specializing in skills specific to frontier AI development](https://ai-2027.com/research/timelines-forecast#specialization-specializing-in-skills-specific-to-frontier-ai-development) | Same as above, except working on the exact projects pursued within AGI companies. | Eli: 1.7 \[0.5, 6\] Nikola: 0.4 \[0.1, 2\] FutureSearch: 2.4 \[0.5, 4.7\] | Estimated from the fact that fine-tuning for specific use cases usually doesn’t take long, and the overlap between RE-Bench tasks and real-world coding is large. |
| [Cost and speed](https://ai-2027.com/research/timelines-forecast#cost-and-speed) | Same as above, except doing it at a cost and speed such that there are substantially more superhuman AI agents than human engineers (specifically, 30x more agents than there are humans, each one accomplishing tasks 30x faster). | Eli: 6.9 \[1, 48\] Nikola: 6 \[1, 36\] FutureSearch: 13.5 \[4.5, 36\] | Estimated from data of AI capabilities getting cheaper over time. |
| [Other task difficulty gaps](https://ai-2027.com/research/timelines-forecast#cost-and-speed) | SC achieved. | Eli: 5.5 months \[1, 30\] Nikola: 3 months \[0.5, 18\] FutureSearch: 14.7 \[2, 58.8\] | Accounting for unknown unknowns |
...
An AI with an R% time horizon of T time means that it has an average success rate of R% on tasks that take humans T time. For more details about their methodology and their task suite HCAST, see [above](https://ai-2027.com/research/timelines-forecast#metrs-time-horizon-report).
...
As described [above](https://ai-2027.com/research/timelines-forecast#how-high-does-re-bench-go), we set the RE-Bench saturation based on achieving an average of 1.5, which is about what we think the best human could get given 8 hours for each task.
...
1. Describe and show the impact of each model update ( [more](https://ai-2027.com/research/timelines-forecast#model-updates))

2. Examine the importance of each source of superexponential progress ( [more](https://ai-2027.com/research/timelines-forecast#the-importance-of-each-source-of-superexponential-progress))

3. Add a few clarifications regarding the model ( [more](https://ai-2027.com/research/timelines-forecast#clarifications))
...
- Eli’s estimate of gap size: 3 months \[0.5, 18\]. Reasoning:
  ...
  - In the METR suite, scaleup in both modified LOC and files LOC is proportional to time horizon, which is doubling roughly every 4 months in 2025 and may be doubling faster at longer time horizons (see [time horizon forecast above](https://ai-2027.com/research/timelines-forecast#time-horizon)).
...
- Eli’s estimate of gap size: 6.9 months \[1, 48\]. Reasoning:
  ...
  - Looking at RE-Bench, a more agentic benchmark which started at roughly human cost and we can take into account fewer tokens being needed, we see a faster decrease of roughly 10x in .33 years so a 1,000x/year trend (based on [Figure 11](https://arxiv.org/pdf/2411.15114#page=20), comparing 3.5 Sonnet Old to 3.5 Sonnet New).
    ...
    - However, looking at [updated data](https://ai-2027.com/research/timelines-forecast#running-an-extrapolation) it seems like the rate of change between Old and New was unusually large, so this should be discounted.
...
1. An extrapolation of the HCAST suite doesn’t cover gaps that will come up in the real world (poor feedback loops is my guess as to the most important gap, see [here](https://ai-2027.com/research/timelines-forecast#summary-table) for some of the candidates that seem most prominent).
...
Harvard University, part-time intern at [METR](https://metr.org/)

2.

[futuresearch.ai](http://futuresearch.ai/), individual forecaster bios on [futuresearch.ai/ai-2027](http://futuresearch.ai/ai-2027)
...
A full SC needs to do this faster and cheaper as well, but this will be accounted for later on in the time horizon extension method.
